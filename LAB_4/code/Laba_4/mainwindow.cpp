...
bool equalPoints(QPoint p1, QPoint p2) {
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    if(algorithm !=BRESENHAM_CIRCLE)
    {
        ...
        {
            if (i > 0 && !equalPoints(circle[i-1].back(), circle[i][0])) {
                p.drawLine(pix.width()*(0.5+stp*circle[i-1].back().x()),pix.height()*(0.5-stp*circle[i-1].back().y()),pix.width()*(0.5+stp*circle[i][0].x()),pix.height()*(0.5-stp*circle[i][0].y()));
            }
...
QVector<QVector<QPoint>> MainWindow::BresenhamCircle(double x0, double y0, double R){
    ...
    while (y >= x) {
        ...
        circle[1].push_back(QPoint(x0 + y, y0 - x));
        circle[2].push_back(QPoint(x0 + y, y0 + x));
        circle[3].push_back(QPoint(x0 + x, y0 + y));
        circle[4].push_back(QPoint(x0 - x, y0 + y));
        circle[5].push_back(QPoint(x0 - y, y0 + x));
        circle[6].push_back(QPoint(x0 - y, y0 - x));
        circle[7].push_back(QPoint(x0 - x, y0 - y));
