...
def main():
    ...
    for segment in segments:
        x1, y1, x2, y2 = segment
        draw_line(ax1, x1, y1, x2, y2, color='blue')
        ...
        if clipped_start is not None and clipped_end is not None:
            draw_line(ax1, clipped_start[0], clipped_start[1], clipped_end[0], clipped_end[1], color='green')
    ...
    for segment in segments:
        x1, y1, x2, y2 = segment
        draw_line(ax2, x1, y1, x2, y2, color='blue')

        clipped_start, clipped_end = cyrus_beck(x1, y1, x2, y2, polygon_vertices)
        if clipped_start is not None and clipped_end is not None:
            draw_line(ax2, clipped_start[0], clipped_start[1], clipped_end[0], clipped_end[1], color='green')

    draw_polygon(ax2, polygon_vertices, color='orange')
